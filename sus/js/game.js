class Building {
    ...
    generateShopHTML() {
        ...
        let html = `<b>${this.name}</b></br>You have <b>${this.amount}</b> ${this.name.toLowerCase()}(s).</br>Each ${this.name.toLowerCase()} produces <b>${format(singleEffect)}</b> amongus.</br>All of your ${this.name.toLowerCase()}(s) combined produces <b>${format(this.effect)}</b> amongus.</br>${this.generateBuyButtons()}</br>${this.generateUpgradeButtons()}`;
        return html;
    }
}
...
let game = {
    ...
    updateDisplays () {
        ...
        document.title = 'Amongus Clicker | ' + format(player.cookies);
